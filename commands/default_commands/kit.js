module.exports = {
    ...
    async execute(settings, message, args) {
        ...
        let match_id = settings.search_handler('kits', true, args)
        //gets the file of the kit
        const info_file = require('./../../output/kits/' + match_id.toString() + '.json')
        //creates an embed with a title link: `objects/item-sets/${match_id}` which is automatically appended to the lu explorer link
        let embed = settings.embed_template(settings.config, info_file.name, `objects/item-sets/${match_id}`, info_file.iconURL)
        //gets all fields
        let fields = settings.field_handler(settings, info_file, 'kits', false, 'unused')
